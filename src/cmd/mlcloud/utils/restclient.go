...
import (
	...
	printutils "github.com/deepinsight/tf-operator/pkg/util"
)
...
func MakeRequestToken(uri string, method string, body io.Reader,
	...
	if err != nil {
		return nil, fmt.Errorf("MakeRequestToken: %v", err)
	}
...
func PostCall(targetURL string, jsonString []byte) ([]byte, error) {
	glog.V(4).Info(printutils.Pformat(jsonString))
...
func PostChunk(targetURL string,
	...
	log.Debugf("PostChunk chunkName: %s", chunkName)
