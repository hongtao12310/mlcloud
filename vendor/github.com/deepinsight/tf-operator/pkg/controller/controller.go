...
import (
	...
	"github.com/deepinsight/tf-operator/pkg/trainer"
...
func (c *Controller) handleTfJobEvent(event *Event) error {
	...
	case kwatch.Deleted:
        ...
        // [Hongtao] we already received delete signal, just delete the job metadata from memory
        // no need to send the delete event
	...
	go func() {
		...
		for {
			...
			for {
                ...
                // [Hongtao] if we received delete event. we don't need to watch the resource Version again
