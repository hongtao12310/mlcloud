...
import (
	...
	"github.com/deepinsight/tf-operator/pkg/util/retryutil"
...
func (j *TrainingJob) GetStatus() (spec.State, []*spec.TfReplicaStatus, error) {
	...
	// [Hongtao] we have a case to run only one worker task
    if v, ok := replicaSetStates[spec.MASTER]; ok {
        if v == spec.ReplicaStateSucceeded {
            state = spec.StateSucceeded
            return state, replicaStatuses, nil
        } else if v == spec.ReplicaStateFailed {
            state = spec.StateFailed
            return state, replicaStatuses, nil
        }
    } else if w, ok := replicaSetStates[spec.WORKER]; ok {
        if w == spec.ReplicaStateSucceeded {
            state = spec.StateSucceeded
            return state, replicaStatuses, nil
        } else if w == spec.ReplicaStateFailed {
            state = spec.StateFailed
            return state, replicaStatuses, nil
        }
    }
