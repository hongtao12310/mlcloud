...
func (s *TBReplicaSet) Delete() error {
	...
	err = s.ClientSet.ExtensionsV1beta1().Ingresses(s.Job.job.Metadata.Namespace).Delete(s.jobName(), &meta_v1.DeleteOptions{})
	if err != nil {
		log.Errorf("Error deleting ingress: %v; %v", s.jobName(), err)
		failures = true
	}
